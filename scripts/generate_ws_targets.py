...
def generate_pretext_xml_tree(activities_dir):
    """
    ...
    Args:
        activities_dir (str): The path to the directory containing the source files.
    ...
    for root, dirs, files in os.walk(activities_dir):
        for filename in files:
            if filename.startswith("ws_") and filename.endswith(".ptx"):
...
if __name__ == "__main__":
    activities_directory = "source/activities"
    
    if not os.path.isdir(activities_directory):
        print(f"Error: The directory '{activities_directory}' was not found.")
        print("Please ensure this script is in the same folder as your `activities` directory.")
    else:
        xml_tree = generate_pretext_xml_tree(activities_directory)
